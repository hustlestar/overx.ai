...
export class ContentOptimizer {
  ...
  analyzeContent(
    ...
    contentLength: { words: number; recommendation: string }
    suggestions: string[]
  } {
    ...
    const missingSecondary = secondaryKeywords.filter(kw => 
  ...
  analyzeHeadings(headings: Array<{ level: number; text: string }>, targetKeyword?: string): OptimizationResult {
    const suggestions: string[] = []
    const issues: string[] = []
    let score = 100
